...
async function main() {
    ...
    let election = await Election.findOne({ electionCode: "ELECTION_123" });
    if (!election) {
        console.log("Election ELECTION_123 not found in db. Let's try finding the first election available.");
        election = await Election.findOne({});
        if(!election) {
            console.log("No election found at all.");
            process.exit(1);
        }
        console.log("Found another election: ", election.electionCode)
    }
    console.log("Found Election:", election.electionCode, "| Current contract Address:", election.contractAddress);
